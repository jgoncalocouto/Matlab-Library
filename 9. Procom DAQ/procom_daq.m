function [data] = procom_daq(port,varargin)
%procom_daq: Function to acquire data from PROCOM either in a continuous or during a specified time period

%   Inputs:
%   1. port - Serial port name where the appliance is connected (required)
%   2. (...,'acquisition_time',t_daq) - Time rate of data acquisition [s] (optional) 
%   3. (...,'acquisition_mode',{'continuous','timer'},{if 'timer': timer_period}) - Acquisition mode
%       3.1 - Acquisition mode: 'continuous' (procom_daq continously acquire data until the figure is closed) E.g. procom_daq('COM3','acquisition_mode','continuous')
%       3.2 - Acquisition mode: 'timer' (procom_daq acquires data during a specified timer_period [s]) E.g. procom_daq('COM3','acquisition_mode','timer',30)
%   4. (...,'time_axis_size',t_size) - command to specify the size of slidding time window t_size [s]

% Outputs
%   1. data - Timetable with the data acquired from procom
%% PROCOM DAQ: Initialization
instrreset


acquisition_time_available=0;
acquisition_mode_available=0;
time_axis_size_available=0;
figure_status=1;

if nargin>1
    for i=1:size(varargin,2)
        if strcmp(varargin{i},'acquisition_time')==1
            t_daq=varargin{i+1};
            acquisition_time_available=1;
        elseif strcmp(varargin{i},'acquisition_mode')==1
            acquisition_mode=varargin{i+1};
            acquisition_mode_available=1;
            if strcmp(acquisition_mode,'timer')==1
                timer_period=varargin{i+2};
            end
        elseif strcmp(varargin{i},'time_axis_size')==1
            time_axis_size=varargin{i+1};
            time_axis_size_available=1;
        elseif strcmp(varargin{i},'figureOFF')
            figure_status=0;
        end
    end
end

if acquisition_time_available==0
    t_daq=1; %default value for acquisition time is 1[s]
end

if acquisition_mode_available==0
    acquisition_mode='continuous'; % default acquisition mode
end

if time_axis_size_available==0
    time_axis_size=60; % default slidding window time duration is 60[s]
end
...
elseif figure_status==0
  ProcomFigHandle = figure('Visible', 'off','units','normalized','outerposition',[0.0 0.0 0.4 0.8]);
set(ProcomFigHandle,'Name','Procom Live','NumberTitle','off','DoubleBuffer', 'on','CurrentCharacter','a');   
end
...
hold on
T_win_Handle = plot(nan,'c');
T_wout_Handle = plot(nan,'r');
T_wmix_Handle = plot(nan,'r');
...
%% PROCOM Acquisition Session Configurations
    ...
    elseif record==1
        if carry==125 && buffer(i-1)==125 && false_finish==0
            ...
            if buffer(6)==hex2dec('A0') % Checks if message of the type: broadcast
                ...
                if k-time_axis_size*(1/t_daq)<1
                    a=1;
                    xlim_start=0;
                    xlim_end=time_axis_size+10;
                else
                    a=round(k-time_axis_size*(1/t_daq),0);
                ...
                set(T_wmix_Handle  ,'YData', T_wmix(a:k) );
            ...
            if strcmp(acquisition_mode,'timer')==1
                if seconds(datetime(now,'ConvertFrom','datenum')-t_ref)>=timer_period
