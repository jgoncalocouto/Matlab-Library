...
script_input='instruction_script';
...
I_fan=zeros(buffer_size,1); %Fan Current [A] - Lixo
...
%% Aquisition Routine
while msg_ready==0 && ishandle(ProcomFigHandle)==1
    
    carry=fscanf(s,'%c',1); %get a single character
    
    % Handle exception: '{' found, and next character is not a '{'
    if start_supervision==1 && carry==123 && record==0 %if a '{' has been detected before and there is another, start recording
        buffer(i-1)=123;
        buffer(i)=123;
        i=i+1;
        start_supervision=0;
        record=1;
    elseif start_supervision==1 && carry~=123 && record==0 %if not, delete previously stored '{'
        start_supervision=0;
    ...
    if carry==123 && start_supervision==0 && record==0 %Detect a '{' and raise a flag
        i=i+1;
        carry=0;
        start_supervision=1; %flag
        
    elseif record==1 && carry==125 && buffer(i-1)==125 %If record flag is up and two consecutive '}' have been detected, terminate recording, treat values, store them and update plots
        buffer(i)=carry;
        i=i+1;
        record=0;
        msg_ready=1;
        ...
        if buffer(6)==hex2dec('A0')
            ...
            T_wout(k,1)=buffer(10)+buffer(11)./100; %Outlet Water Temperature -HEx
            ...
            T_wmix(k,1)=buffer(35)+buffer(36)./100; %Mix Temperature
